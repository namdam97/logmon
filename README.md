...
- **Clean Architecture** — `user`, `order` (CRUD, domain đơn giản).
- **Clean Arch + DDD + CQRS** — `alerting`, `slo`, `logpipeline` (business rule phức tạp).
...
| Đã có | Đang/ sẽ làm |
|-------|--------------|
| `user` BC: đăng ký / đăng nhập / đăng xuất / `/me` (JWT cookie, bcrypt, rate limit) | `order`, `alerting`, `slo`, `logpipeline` BC |

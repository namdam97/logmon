// Package postgres implement persistence ports của notification BC trên PostgreSQL
// (pgx/v5). Transaction dùng pattern tx-in-context (giống slo/alerting BC).
// Config kênh (chứa secret) được mã hóa CẢ blob qua shared/crypto.Cipher.
package postgres
...
// TxManager chạy fn trong một transaction; tx gắn vào ctx cho adapter khác.
